#!/usr/bin/env python 
# -*- coding: utf-8 -*- 
# @Time : 2019/5/22 上午1:16 
# @Author : Gaoxiang Chen
# @Site :  
# @File : 355. Design Twitter.py 
# @Software: PyCharm
# ---------------------
...
class Twitter(object):
    ...
    def getNewsFeed(self, userId):
        ...
        all_tweets = []
        for u in followees:
            for (time, tweet) in self.tweets[u]:
                all_tweets.append((-time, tweet))
        from heapq import heappop, heapify
        heapify(all_tweets)
        res = []
        while len(res) < 10 and all_tweets:
            res.append(heappop(all_tweets)[1])
        return res
